...
{{ customParams.proxyName.JapanSelect }} = {{ "smart" if customParams.isSmart else "select" }}, no-alert=0, hidden=0, policy-regex-filter=(?=.*(🇯🇵|日本|川日|东京|大阪|泉日|埼玉|JP|Japan).)^((?!(4x)).)*$, include-all-proxies=0, include-other-group="{{ customParams.proxyName.SubNodesSelect }}"
